...
> Q. 팔을 뻗을 때 운동피질 뉴런들은 왜 회전 패턴을 만드는가?
...
고차원 신경 궤적에서, 순수하게 회전만 하는 것처럼 보이는 2D 평면을 찾아주는 방법
...
- 뇌에는 방정식이 없기 때문에 고정점 분석이 불가하다.
- RNN는 직접 만든 것이기 때문에 고정점 분석이 가능하다.
...
- 과제로 학습했음에도 불구하고 RNN이 스스로 회전을 만들어낸다.
...
| 단계 | 핵심 결과 |
|---|---|
| **M0** | 레퍼런스 대비 평면 각도 **0.0008°**, 주파수 오차 **0.000%**<br>합성 데이터 적합 R² **0.8552** (무제약 상한 0.9886), 평면 분산 **0.9021** |
| **M1** | 과제 MSE **0.0086** (<0.02) · 고유 고정점 27개 → **안정 8개 + 안장 19개**<br>8개가 **서로 다른 8개 큐브 꼭짓점**, 판독값 모두 ±1의 0.05 이내 |
| **M2** | 실데이터 (108조건 × 61시점 × 218뉴런)<br>적합 R² **0.5236** · 회전 평면 분산 **0.4643** · 주파수 **1.37 Hz** |
| **M3** | 속도 R² **0.9989** (게이트 >0.90)<br>은닉 상태 jPCA 적합 R² **0.884** · 평면 분산 0.149 → **회전 창발** |
| **M4** | 고정점 **1개**: `|x|=2.58`, **회전 안장점(spiral)**, 주고유쌍 **0.51 ± 1.33i**<br>흐름장 회전 성분(curl) **+0.96** 
| **M5** | 자체완결 뷰어 124 KB (외부 라이브러리 0) · 3개 뷰 · 고정점 호버 툴팁 |
...
**파이프라인.**
Soft normalize - Subtract CC mean - PCA - 유한차분 - 반대칭 최소제곱 - 고유분해
...
고유값: 실수부는 팽창, 수축 정도, 허수부는 회전 속도를 나타냄..
